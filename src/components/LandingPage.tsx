...
// === Main Component ===
export function LandingPage({ onEnterBuilder }: LandingPageProps) {
      ...
      <nav className="h-14 flex items-center justify-between px-6 border-b border-zinc-800/50 bg-zinc-950/90 backdrop-blur-xl shrink-0 z-50">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex items-center gap-5">
            {["Features", "3D Builder", "Pricing", "Contact"].map((item) => (
              <button
                key={item}
                onClick={item === "3D Builder" ? onEnterBuilder : undefined}
                className={`text-sm transition-colors ${item === "3D Builder" ? "text-white font-medium" : "text-zinc-500 hover:text-zinc-300"}`}
              >
                {item}
              </button>
            ))}
